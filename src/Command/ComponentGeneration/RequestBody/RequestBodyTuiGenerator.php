...
class RequestBodyTuiGenerator extends AbstractTuiComponentGenerator
{
    ...
    private function generateComponent(RequestBodyTuiState $state): void
    {
        ...
        if ('php' === $state->format_output || 'both' === $state->format_output) {
            $array = $this->mergeWithExistingPhp($array, $state->name, $destination, 'addRequestBody');
    ...
    private function deleteComponent(RequestBodyTuiState $state): void
    {
        ...
        if (str_ends_with($state->loadedFrom, '.php')) {
            ...
            $this->currentOutput->writeln(sprintf('<info>Request Body "%s" deleted from %s</info>', $state->name, $state->loadedFrom));

            return;
        }

        $existingConfig = \Symfony\Component\Yaml\Yaml::parseFile($state->loadedFrom);
        if (!isset($existingConfig['documentation']['components']['requestBodies'])) {
            return;
        }

        unset($existingConfig['documentation']['components']['requestBodies'][$state->name]);
