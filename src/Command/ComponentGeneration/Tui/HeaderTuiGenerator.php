...
class HeaderTuiGenerator extends AbstractTuiComponentGenerator
{
    ...
    {
        return 'Générer un composant d\'en-tête HTTP';
    }
    ...
    private function showComponentList(Tui $tui, callable $onBack): void
    {
        ...
        $container->add(new TextWidget($this->currentOutput->getFormatter()->format("<info>Header — Sélection ou création</info>\n")));
    ...
    private function showForm(Tui $tui, HeaderTuiState $state, callable $onBack): void
    {
        ...
        $settingItems = [];
        $settingItems[] = new SettingItem('name', 'Nom', $state->name, 'Nom du header (ex: X-Request-ID)', [], $textInputCallback);
        $settingItems[] = new SettingItem('desc', 'Description', $state->description, 'Description du header', [], $textInputCallback);
        ...
        $container = new ContainerWidget();
        $container->expandVertically(true);
        $container->add(new TextWidget($this->currentOutput->getFormatter()->format("<info>Configuration Header</info>\n")));
    ...
    private function generateComponent(HeaderTuiState $state): void
    {
        ...
        $destination = AbstractGenerateComponentCommand::COMPONENT_HEADERS;
        ...
        $this->currentOutput->writeln(sprintf('<info>Header "%s" généré avec succès dans %s</info>', $state->name, $dumpLocation));
    }
}
