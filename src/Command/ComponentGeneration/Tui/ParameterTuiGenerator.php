...
class ParameterTuiGenerator extends AbstractTuiComponentGenerator
{
    ...
    {
        return 'Générer un composant de paramètre de requête';
    }
    ...
    private function showComponentList(Tui $tui, callable $onBack): void
    {
        ...
        $container->add(new TextWidget($this->currentOutput->getFormatter()->format("<info>Parameter — Sélection ou création</info>\n")));
    ...
    private function showForm(Tui $tui, ParameterTuiState $state, callable $onBack): void
    {
        ...
        $settingItems = [];
        $settingItems[] = new SettingItem('name', 'Nom', $state->name, 'Nom du paramètre', [], $textInputCallback);
        $settingItems[] = new SettingItem('in', 'In', $state->in, 'Localisation du paramètre', ['query', 'path', 'header', 'cookie']);
        $settingItems[] = new SettingItem('desc', 'Description', $state->description, 'Description du paramètre', [], $textInputCallback);
        $settingItems[] = new SettingItem('required', 'Requis', $state->required ? 'oui' : 'non', 'Paramètre requis', ['oui', 'non']);
        $settingItems[] = new SettingItem('schema_type', 'Type', $state->schemaType, 'Type schéma', ['string', 'integer', 'number', 'boolean', 'array']);
        $settingItems[] = new SettingItem('format', 'Format', $state->format, 'Format (optionnel)', [], $textInputCallback);
        $settingItems[] = new SettingItem('example', 'Exemple', $state->example, 'Valeur exemple', [], $textInputCallback);
        ...
        $container = new ContainerWidget();
        $container->expandVertically(true);
        $container->add(new TextWidget($this->currentOutput->getFormatter()->format("<info>Configuration Parameter</info>\n")));
    ...
    private function generateComponent(ParameterTuiState $state): void
    {
        ...
        $this->currentOutput->writeln(sprintf('<info>Parameter "%s" généré avec succès dans %s</info>', $state->name, $dumpLocation));
    }
}
